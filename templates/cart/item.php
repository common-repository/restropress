	...
	<div>
		<span class="cart-action-wrap">
			<a class="rpress-edit-from-cart" data-cart-item="{cart_item_id}" data-item-name="{item_title}" data-item-id="{item_id}" data-item-price="{item_amount}" data-remove-item="{edit_food_item}">
				<span class="rp-ajax-toggle-text"><?php echo esc_html( apply_filters( 'rpress_cart_edit', __('Edit', 'restropress' ) ) ); ?></span>
			</a>
			<a href="{remove_url}" data-cart-item="{cart_item_id}" data-fooditem-id="{item_id}" data-action="rpress_remove_from_cart" class="rpress-remove-from-cart"><?php echo esc_html( apply_filters( 'rpress_cart_remove', __('Remove', 'restropress' ) ) ); ?></a>
		</span>
	</div>
</li>