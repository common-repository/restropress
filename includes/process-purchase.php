...
function rpress_process_purchase_form() {
	...
	$purchase_key     = strtolower( md5( $user['user_email'] . gmdate( 'Y-m-d H:i:s' ) . $auth_key . uniqid( 'rpress', true ) ) );
...
function rpress_checkout_check_existing_email( $valid_data, $post ) {
    ...
    if ( is_user_logged_in() ) {
        $email = isset( $valid_data['logged_in_user']['user_email'] ) ? strtolower( $valid_data['logged_in_user']['user_email'] ) : null;
        if ( $email !== null ) { // Check if email is not null
            $customer = new RPRESS_Customer( get_current_user_id(), true );
            // Check if $customer->email is not null
            $customer_email = $customer->email ? strtolower( $customer->email ) : null;
            // If this email address is not registered with this customer, see if it belongs to any other customer
            if ( $email != $customer_email && ( is_array( $customer->emails ) && ! in_array( $email, array_map( 'strtolower', $customer->emails ) ) ) ) {
