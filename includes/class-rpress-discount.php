...
/**
 * RPRESS_Discount Class
 *
 * @since 1.0.6
 */
class RPRESS_Discount {
	/**
	 * Discount ID.
	 *
	 * @since 1.0.6
	...
	/**
	 * Converts the instance of the RPRESS_Discount object into an array for special cases.
	 *
	 * @since 1.0.6
	 *
	 * @return array RPRESS_Discount object as an array.
	 */
	...
	private function find_by_code( $code = '' ) {
		...
		$discounts = rpress_get_discounts(
			array(
				'meta_key'       => '_rpress_discount_code',
				'meta_value'     => $code,
				'posts_per_page' => 1,
				'post_status'    => 'any',
				'fields'         => 'ids'
	...
	private function setup_discount( $discount = null ) {
		...
		if ( 'rpress_discount' !== $discount->post_type ) {
			return false;
		}
		/**
		 * Fires before the instance of the RPRESS_Discount object is set up.
		 *
		 * @since 1.0.6
		 *
		 * @param object RPRESS_Discount      RPRESS_Discount instance of the discount object.
		 * @param object WP_Post $discount WP_Post instance of the discount object.
		 */
		do_action( 'rpress_pre_setup_discount', $this, $discount );
		/**
		...
		/**
		 * Fires after the instance of the RPRESS_Discount object is set up. Allows extensions to add items to this object via hook.
		 *
		 * @since 1.0.6
		 *
		 * @param object RPRESS_Discount      RPRESS_Discount instance of the discount object.
		 * @param object WP_Post $discount WP_Post instance of the discount object.
		 */
		do_action( 'rpress_setup_discount', $this, $discount );
		return true;
	}
	/**
	 * Setup Functions
	...
	public function get_meta( $key = '', $single = true ) {
		$meta = get_post_meta( $this->ID, '_rpress_discount_' . $key, $single );
		return $meta;
	}
	/**
	...
	public function get_code() {
		 ...
		 */
		return apply_filters( 'rpress_get_discount_code', $this->code, $this->ID );
	}
	/**
	...
	public function get_status_label() {
		 ...
		 * @param int    $ID     Discount ID.
		 */
		return apply_filters( 'rpress_get_discount_status_label', $label, $this->status, $this->ID );
	}
	/**
	...
	public function get_type() {
		 ...
		 * @param int    $ID   Discount ID.
		 */
		return apply_filters( 'rpress_get_discount_type', $this->type, $this->ID );
	}
	/**
	...
	public function get_amount() {
		 ...
		 * @param int    $ID    Discount ID.
		 */
		return (float) apply_filters( 'rpress_get_discount_amount', $this->amount, $this->ID );
	}
	/**
	...
	public function get_product_reqs() {
		...
		/**
		 * Filters the fooditem requirements.
		 *
		 * @since 1.0.6
		 *
		 ...
		 * @param int   $ID           Discount ID.
		 */
		return (array) apply_filters( 'rpress_get_discount_product_reqs', $this->product_reqs, $this->ID );
	}
	/**
	 * Retrieve the fooditems that are excluded from having this discount code applied.
	 *
	 * @since 1.0.6
	 *
	 * @return array IDs of excluded fooditems.
	 */
	...
	public function get_start() {
		 ...
		 * @param int    $ID    Discount ID.
		 */
		return apply_filters( 'rpress_get_discount_start', $this->start, $this->ID );
	}
	/**
	...
	public function get_expiration() {
		 ...
		 * @param int   $ID          Discount ID.
		 */
		return apply_filters( 'rpress_get_discount_expiration', $this->expiration, $this->ID );
	}
	/**
	...
	public function get_uses() {
		 ...
		 * @param int $ID       Discount ID.
		 */
		return (int) apply_filters( 'rpress_get_discount_uses', $this->uses, $this->ID );
	}
	/**
	...
	public function get_max_uses() {
		 ...
		 * @param int $ID       Discount ID.
		 */
		return (int) apply_filters( 'rpress_get_discount_max_uses', $this->max_uses, $this->ID );
	}
	/**
	...
	public function get_min_price() {
		 ...
		 * @param int   $ID        Discount ID.
		 */
		return (float) apply_filters( 'rpress_get_discount_min_price', $this->min_price, $this->ID );
	}
	/**
	...
	public function get_is_single_use() {
		 ...
		 * @param int  $ID            Discount ID.
		 */
		return (bool) apply_filters( 'rpress_is_discount_single_use', $this->is_single_use, $this->ID );
	}
	/**
	...
	public function get_is_not_global() {
		 ...
		 * @param int  $ID            Discount ID.
		 */
		return (bool) apply_filters( 'rpress_discount_is_not_global', $this->is_not_global, $this->ID );
	}
	/**
	...
	public function get_product_condition() {
		 ...
		 * @param int    $ID                Discount ID.
		 */
		return apply_filters( 'rpress_discount_product_condition', $this->product_condition, $this->ID );
	}
	/**
	...
	private function insert_discount() {
		...
		if ( ! empty( $discount_data['start'] ) ) {
			$discount_data['start'] = gmdate( 'm/d/Y H:i:s', $start_timestamp );
		}
		if ( ! empty( $discount_data['expiration'] ) ) {
			$discount_data['expiration'] = gmdate( 'm/d/Y H:i:s', strtotime( gmdate( 'm/d/Y', strtotime( $discount_data['expiration'] ) ) . ' 23:59:59' ) );
		...
		$args = apply_filters( 'rpress_insert_discount_args', array(
		...
		// Create a blank rpress_discount post
	...
	public function save() {
		...
		if ( true == $saved ) {
			 ...
			 * @since 1.0.6
			 *
			 * @param object       Instance of RPRESS_Discount object.
			 * @param string $key  Meta key.
			 */
			do_action( 'rpress_discount_save', $this->ID, $this );
		}
		return $saved;
	}
	/**
	...
	public function add( $args ) {
		...
		if ( ! empty( $this->ID ) && $this->exists() ) {
			 ...
			 * @param int   $ID   Discount ID.
			 */
			$meta = apply_filters( 'rpress_insert_discount', $meta );
			/**
			 ...
			 * @param array $meta Discount meta.
			 */
			do_action( 'rpress_pre_insert_discount', $meta );
			 ...
			 */
			do_action( 'rpress_post_insert_discount', $meta, $this->ID );
			$this->setup_discount( WP_Post::get_instance( $this->ID ) );
	...
	public function update( $args ) {
		 ...
		 * @param int   $ID   Discount ID.
		 */
		$meta = apply_filters( 'rpress_update_discount', $meta, $this->ID );
		/**
		 ...
		 * @param int   $ID   Discount ID.
		 */
		do_action( 'rpress_pre_update_discount', $meta, $this->ID );
		wp_update_post( array(
			'ID'          => $this->ID,
			'post_title'  => $meta['name'],
		 ...
		 * @param int   $ID   Discount ID.
		 */
		do_action( 'rpress_post_update_discount', $meta, $this->ID );
		return $this->ID;
	}
	/**
	...
	private function build_meta( $args = array() ) {
		...
		if ( ! empty( $meta['start'] ) ) {
			$meta['start']      = gmdate( 'm/d/Y H:i:s', $start_timestamp );
		}
		if ( ! empty( $meta['expiration'] ) ) {
			$meta['expiration'] = gmdate( 'm/d/Y H:i:s', strtotime( gmdate( 'm/d/Y', strtotime( $meta['expiration'] ) ) . ' 23:59:59' ) );
	...
	public function update_status( $new_status = 'active' ) {
		 ...
		 */
		do_action( 'rpress_pre_update_discount_status', $this->ID, $new_status, $this->post_status );
		$id = wp_update_post(
			array(
				'ID'          => $this->ID,
		 ...
		 * @param string $post_status Post status.
		 */
		do_action( 'rpress_post_update_discount_status', $this->ID, $new_status, $this->post_status );
		if ( $id == $this->ID ) {
			return true;
		}
		return false;
	...
	public function is_started( $set_error = true ) {
		 ...
		 * @param int  $ID     Discount ID.
		 */
		return apply_filters( 'rpress_is_discount_started', $return, $this->ID );
	}
	/**
	...
	public function is_expired( $update = true ) {
		 ...
		 * @param int  $ID     Discount ID.
		 */
		return apply_filters( 'rpress_is_discount_expired', $return, $this->ID );
	}
	/**
	...
	public function is_maxed_out( $set_error = true ) {
		 ...
		 * @param int  $ID     Discount ID.
		 */
		return apply_filters( 'rpress_is_discount_maxed_out', $return, $this->ID );
	}
	/**
	...
	public function is_min_price_met( $set_error = true ) {
		$return = false;
		$cart_amount = rpress_get_cart_discountable_subtotal( $this->ID );
		 ...
		 * @param int  $ID     Discount ID.
		 */
		return apply_filters( 'rpress_is_discount_min_met', $return, $this->ID );
	}
	/**
		...
		if ( ! $return && ! empty( $product_reqs ) ) {
			switch ( $this->product_condition ) {
				case 'all' :
					...
					$return = true;
					foreach ( $product_reqs as $fooditem_id ) {
...
/**
	...
	public function is_used( $user = '', $set_error = true ) {
		 ...
		 * @param string $user   User info.
		 */
		return apply_filters( 'rpress_is_discount_used', $return, $this->ID, $user );
	}
	/**
	...
	public function is_valid( $user = '', $set_error = true ) {
		$return = false;
		$user = trim( $user );
		if ( rpress_get_cart_contents() && $this->ID ) {
			if (
		 ...
		 * @param string $user   User info.
		 */
		return apply_filters( 'rpress_is_discount_valid', $return, $this->ID, $this->code, $user );
	}
	/**
	...
	public function get_discounted_amount( $base_price ) {
		 ...
		 */
		return apply_filters( 'rpress_discounted_amount', $amount );
	}
	/**
	...
	public function increase_usage() {
		 ...
		 * @param string $code Discount code.
		 */
		do_action( 'rpress_discount_increase_use_count', $this->uses, $this->ID, $this->code );
		return $this->uses;
	}
	/**
	...
	public function decrease_usage() {
		 ...
		 * @param string $code Discount code.
		 */
		do_action( 'rpress_discount_decrease_use_count', $this->uses, $this->ID, $this->code );
		return $this->uses;
	}
	/**
	 * Edit Discount Link.
