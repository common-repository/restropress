...
function rpress_process_profile_editor_remove_email() {
  ...
  if ( $customer->remove_email( sanitize_email( $_GET['email'] ) ) ) {
    ...
    if (isset($_GET['email']) && isset($user_login)) {
      $email_note = isset($_GET['email']) ? sanitize_email($_GET['email']) : '';
      $user_login_sanitized = isset($user_login) ? sanitize_text_field($user_login) : '';
      // translators: %1$s is a placeholder for the email address and %2$s is a placeholder for the user login.
      $customer_note = sprintf( __('Email address %1$s removed by %2$s', 'restropress'), $email_note, $user_login_sanitized );
