 ...
 * @since 1.0
 */
class RPRESS_Payments_Query extends RPRESS_Stats {
	/**
	 * The args to pass to the rpress_get_payments() query
	 *
	 * @var array
	 * @since 1.0
	 */
	...
	public function get_payments() {
		...
		if ( $query->have_posts() ) {
			while ( $query->have_posts() ) {
				...
				$this->payments[] = apply_filters( 'rpress_payment', $payment, $payment_id, $this );
			}
		}
		add_action( 'rpress_post_get_payments', array( $this, 'date_filter_post' ) );
	...
	public function orderby() {
		switch ( $this->args['orderby'] ) {
			case 'amount' :
				...
				$this->__set( 'meta_key', '_rpress_payment_total' );
			break;
			default :
	...
	public function user() {
		...
		if ( is_numeric( $this->args['user'] ) ) {
			$user_key = '_rpress_payment_user_id';
		} else {
			$user_key = '_rpress_payment_user_email';
		}
	...
	public function customer() {
		...
		$this->__set( 'meta_query', array(
			'key'   => '_rpress_payment_customer_id',
	...
	public function search() {
		...
		if ( ! empty( $this->args['search_in_notes'] ) ) {
			$notes = rpress_get_payment_notes( 0, $search );
			if( ! empty( $notes ) ) {
			...
			$key = $is_email ? '_rpress_payment_user_email' : '_rpress_payment_purchase_key';
			$search_meta = array(
				'key'     => $key,
				'value'   => $search,
				'compare' => 'LIKE'
			...
			rpress_get_option( 'enable_sequential' ) &&
			(
				false !== strpos( $search, rpress_get_option( 'sequential_prefix' ) ) ||
				false !== strpos( $search, rpress_get_option( 'sequential_postfix' ) )
			)
		) {
			$search_meta = array(
				'key'     => '_rpress_payment_number',
			...
			if( is_object( $post ) && $post->post_type == 'rpress_payment' ) {
				$arr   = array();
				$arr[] = $search;
	...
	public function fooditem() {
		if ( empty( $this->args['fooditem'] ) )
			return;
		global $rpress_logs;
		$args = array(
			'post_parent'            => $this->args['fooditem'],
			'log_type'               => 'sale',
		...
		if ( is_array( $this->args['fooditem'] ) ) {
			unset( $args['post_parent'] );
			$args['post_parent__in'] = $this->args['fooditem'];
		}
		$sales = $rpress_logs->get_connected_logs( $args );
		if ( ! empty( $sales ) ) {
			$payments = array();
			foreach ( $sales as $sale ) {
				$payments[] = get_post_meta( $sale, '_rpress_log_payment_id', true );
			}
