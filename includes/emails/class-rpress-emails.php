...
class RPRESS_Emails {
	...
	public function send( $to, $subject, $message, $attachments = '' ) {
		if ( ! did_action( 'init' ) && ! did_action( 'admin_init' ) ) {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'You cannot send email with RPRESS_Emails until init/admin_init has been reached', 'restropress' ), null );
			return false;
		}
		/**
		...
		if( ! $sent && true === $log_errors ) {
			...
			$log_message = sprintf(
				/* translators: 1: Send time, 2: Recipient email address, 3: Email subject */
