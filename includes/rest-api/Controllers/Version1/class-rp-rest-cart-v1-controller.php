...
class RP_REST_Cart_V1_Controller extends WP_REST_Controller {
	...
	public function prepare_item_for_database( $request ): array {
		...
		foreach ( $cart_details as $cart_key => $cart_value ) {
			...
			if ( ! empty( $schema_properties['variation_name'] ) && isset( $cart_details[ $cart_key ]['variation_name'] ) ) {
				$prepared_post->variation_name = $cart_value['variation_name'];
			}
			// Adding sku
			if ( ! empty( $schema_properties['sku'] ) && isset( $cart_details[ $cart_key ]['sku'] ) ) {
				$prepared_post->sku = $cart_value['sku'];
			}
	...
	public function update_cart_content( WP_REST_Request $request ) {
		...
		$response        = new WP_REST_Response( array( 'message' => apply_filters( 'cart_empty_message', __( 'Cart is empty', 'restropress' ) ) ) );
