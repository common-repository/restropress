...
class RPRESS_API {
	...
	public function __construct() {
		...
		$this->log_requests = apply_filters( 'rpress_api_log_requests', $this->log_requests );
	...
	private function validate_request() {
		...
		if ( ! empty( $wp_query->query_vars['rpress-api'] ) && ( $wp_query->query_vars['rpress-api'] != 'products' || ! empty( $wp_query->query_vars['token'] ) ) ) {
			...
			if ( ! ( $user = $this->get_user( $wp_query->query_vars['key'] ) ) ) :
				...
				$secret = get_user_meta( $user, 'rpress_user_secret_key', true );
	...
	public function process_query() {
		...
		if ( ! isset( $wp_query->query_vars['rpress-api'] ) )
			return;
		...
		$this->data = apply_filters( 'rpress_api_output_data', $data, $query_mode, $this );
	...
	*/
	...
	public function get_customers( $customer = null ) {
		...
		if( $customer_query ) {
			foreach ( $customer_query as $customer_obj ) {
				...
				$customers['customers'][$customer_count]['stats']['total_fooditems'] = rpress_count_file_fooditems_of_user( $customer_obj->email );
				$customer_count++;
			}
		} elseif( $customer ) {
	...
	public function get_products( $product = null ) {
		...
		if ( $product == null ) {
			...
			if ( get_post_type( $product ) == 'fooditem' ) {
				...
				if ( $args['date'] == null ) {
					...
					while( $y <= $dates['year_end'] ) :
						...
						while ( $i <= $month_end ) :
							...
							while ( $d <= $num_of_days ) :
								$sale_count = rpress_get_sales_by_date( $d, $i, $y );
					...
					if( $args['date'] == 'this_quarter' || $args['date'] == 'last_quarter'  ) {
						...
						while( $month <= $dates['m_end'] ) :
							$sales_count += rpress_get_sales_by_date( null, $month, $dates['year'] );
							$month++;
						endwhile;
						$sales['sales'][ $args['date'] ] = $sales_count;
   					} else {
						$sales['sales'][ $args['date'] ] = rpress_get_sales_by_date( $dates['day'], $dates['m_start'], $dates['year'] );
   					}
				}
				...
				if ( $args['date'] == null ) {
					...
					while( $y <= $dates['year_end'] ) :
						...
						while ( $i <= $month_end ) :
							...
							while ( $d <= $num_of_days ) :
								$earnings_stat = rpress_get_earnings_by_date( $d, $i, $y );
								$date_key = gmdate( 'Ymd', strtotime( $y . '/' . $i . '/' . $d ) );
					...
					if ( $args['date'] == 'this_quarter' || $args['date'] == 'last_quarter'  ) {
						...
						$month = $dates['m_start'];
						while ( $month <= $dates['m_end'] ) :
							$earnings_count += rpress_get_earnings_by_date( null, $month, $dates['year'] );
							$month++;
						endwhile;
						$earnings['earnings'][ $args['date'] ] = $earnings_count;
   					} else {
						$earnings['earnings'][ $args['date'] ] = rpress_get_earnings_by_date( $dates['day'], $dates['m_start'], $dates['year'] );
   					}
				}
			} elseif ( $args['product'] == 'all' ) {
				$products = get_posts( array( 'post_type' => 'fooditem', 'nopaging' => true ) );
	...
	public function get_recent_sales() {
		...
		if( isset( $wp_query->query_vars['id'] ) ) {
			$query   = array();
			$query[] = rpress_get_payment_by( 'id', $wp_query->query_vars['id'] );
		} elseif( isset( $wp_query->query_vars['purchasekey'] ) ) {
			$query   = array();
			$query[] = rpress_get_payment_by( 'key', $wp_query->query_vars['purchasekey'] );
		} elseif( isset( $wp_query->query_vars['email'] ) ) {
			$query = rpress_get_payments( array( 'meta_key' => '_rpress_payment_user_email', 'meta_value' => $wp_query->query_vars['email'], 'number' => $this->per_page(), 'page' => $this->get_paged(), 'status' => 'publish' ) );
		} else {
			$query = rpress_get_payments( array( 'number' => $this->per_page(), 'page' => $this->get_paged(), 'status' => 'publish' ) );
		}
		if ( $query ) {
			$i = 0;
			foreach ( $query as $payment ) {
				...
				$cart_items   = rpress_get_payment_meta_cart_details( $payment->ID );
				$sales['sales'][ $i ]['ID']             = rpress_get_payment_number( $payment->ID );
				$sales['sales'][ $i ]['transaction_id'] = rpress_get_payment_transaction_id( $payment->ID );
				$sales['sales'][ $i ]['key']            = rpress_get_payment_key( $payment->ID );
				$sales['sales'][ $i ]['discount']       = isset( $user_info['discount'] ) && $user_info['discount'] != 'none' ? explode( ',', $user_info['discount'] ) : array();
				$sales['sales'][ $i ]['subtotal']       = rpress_get_payment_subtotal( $payment->ID );
				$sales['sales'][ $i ]['tax']            = rpress_get_payment_tax( $payment->ID );
				$sales['sales'][ $i ]['fees']           = rpress_get_payment_fees( $payment->ID );
				$sales['sales'][ $i ]['total']          = rpress_get_payment_amount( $payment->ID );
				$sales['sales'][ $i ]['gateway']        = rpress_get_payment_gateway( $payment->ID );
				$sales['sales'][ $i ]['email']          = rpress_get_payment_user_email( $payment->ID );
				...
				foreach ( $cart_items as $key => $item ) {
					...
					if ( isset( $item['item_number'] ) && isset( $item['item_number']['options'] ) ) {
						...
						if ( isset( $price_options['price_id'] ) ) {
							$price_name = rpress_get_price_option_name( $item['id'], $price_options['price_id'], $payment->ID );
						}
					}
	...
	public function generate_api_key( $user_id = 0, $regenerate = false ) {
		...
		if ( empty( $user->rpress_user_public_key ) ) {
			update_user_meta( $user_id, 'rpress_user_public_key', $this->generate_public_key( $user->user_email ) );
			update_user_meta( $user_id, 'rpress_user_secret_key', $this->generate_private_key( $user->ID ) );
	...
	public function revoke_api_key( $user_id = 0 ) {
		...
		if ( ! empty( $user->rpress_user_public_key ) ) {
			delete_transient( md5( 'rpress_api_user_' . $user->rpress_user_public_key ) );
			delete_user_meta( $user_id, 'rpress_user_public_key' );
			delete_user_meta( $user_id, 'rpress_user_secret_key' );
		} else {
			return false;
		}
		return true;
	...
	public function update_key( $user_id ) {
		if ( current_user_can( 'edit_user', $user_id ) && isset( $_POST['rpress_set_api_key'] ) ) {
			$user = get_userdata( $user_id );
			if ( empty( $user->rpress_user_public_key ) ) {
				update_user_meta( $user_id, 'rpress_user_public_key', $this->generate_public_key( $user->user_email ) );
				update_user_meta( $user_id, 'rpress_user_secret_key', $this->generate_private_key( $user->ID ) );
	...
	private function get_token( $user_id = 0 ) {
		$user = get_userdata( $user_id );
		return hash( 'md5', $user->rpress_user_secret_key . $user->rpress_user_public_key );
	}
	/**
