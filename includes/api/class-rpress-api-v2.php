 ...
 * @since  2.6
 */
class RPRESS_API_V2 extends RPRESS_API_V1 {
	/**
	...
	public function get_products( $args = array() ) {
		...
		if ( empty( $args['product'] ) ) {
			...
			if( ! empty( $args['category'] ) ) {
				...
				if ( is_numeric( $args['category'] ) ) {
					...
					$query_args['addon_category'] = $args['category'];
				}
			}
			if( ! empty( $args['tag'] ) ) {
				if ( strpos( $args['tag'], ',' ) ) {
				...
				if ( is_numeric( $args['tag'] ) ) {
					...
					$query_args['fooditem_tag'] = $args['tag'];
				}
			}
		...
		return apply_filters( 'rpress_api_products', $products );
	}
	/**
	...
	public function get_product_data( $product_info ) {
		...
		if ( rpress_use_skus() ) {
			$product['info']['sku'] = rpress_get_fooditem_sku( $product['info']['id'] );
		}
		return apply_filters( 'rpress_api_products_product_v2', $product );
	}
	/**
	...
	public function get_customers( $args = array() ) {
		...
		$customer_query = RPRESS()->customers->get_customers( $args );
		$customer_count = 0;
		if( $customer_query ) {
			foreach ( $customer_query as $customer_obj ) {
				// Setup a new RPRESS_Customer object so additional details are defined (like additional emails)
				$customer_obj = new RPRESS_Customer( $customer_obj->id );
			...
			foreach ( $query as $payment ) {
				...
				foreach ( $payment->cart_details as $key => $item ) {
					...
					foreach ( $discount_values as $discount => $amount ) {
						$item_discount = rpress_get_cart_item_discount_amount( $item, $discount );
