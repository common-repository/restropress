...
class RPRESS_Product_Details_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			// translators: %s is a placeholder for the label singular.
			sprintf( __( '%1$s Details', 'restropress' ), rpress_get_label_singular() ),
			array(
				// translators: %s is a placeholder for the label singular.
				'description' => sprintf( __( 'Display the details of a specific %1$s', 'restropress' ), rpress_get_label_singular() ),
			)
		);
	}
	/** @see WP_Widget::widget */
	...
	public function form( $instance ) {
		// Set up some default widget settings.
		$defaults = array(
			// translators: %1$s is a placeholder for the label singular.
			'title'           => sprintf( __( '%1$s Details', 'restropress' ), rpress_get_label_singular() ),
		...
		<label for="<?php echo esc_attr( $this->get_field_id( 'fooditem_id' ) ); ?>">
			<?php 
			/* translators: %s: Label for a food item */
			printf( esc_html__( '%s:', 'restropress' ), esc_html( rpress_get_label_singular() ) ); 
			?>
		</label>
		...
		<?php else: ?>
			<br />
			<input type="text" 
				value="<?php echo esc_attr( $instance['fooditem_id'] ); ?>" 
				placeholder="<?php 
					/* translators: %s: Placeholder for the label singular */
					printf( esc_attr__( '%s ID', 'restropress' ), esc_html( rpress_get_label_singular() ) ); 
				?>" 
				name="<?php echo esc_attr( $this->get_field_name( 'fooditem_id' ) ); ?>" 
				id="<?php echo esc_attr( $this->get_field_id( 'fooditem_id' ) ); ?>"
			>
		<?php endif; ?>
		</p>
		<!-- Download title -->
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'fooditem_title' ) ); ?>">
				<?php 
					/* translators: %s: Placeholder for the label singular */
					printf( esc_html__( 'Show %s Title', 'restropress' ), esc_html( rpress_get_label_singular() ) ); 
				?>
			</label>
		</p>
		...
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'categories' ) ); ?>">
				<?php 
					/* translators: %s: Placeholder for the category name */
					printf( esc_html__( 'Show %s', 'restropress' ), esc_html( $category_labels['name'] ) ); 
				?>
			</label>
		</p>
		<!-- Show fooditem tags -->
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'tags' ) ); ?>">
				<?php 
					/* translators: %s: Placeholder for the tag name */
					printf( esc_html__( 'Show %s', 'restropress' ), esc_html( $tag_labels['name'] ) ); 
				?>
			</label>
		</p>
	...
	public function update( $new_instance, $old_instance ) {
		$instance = $old_instance;
		$instance['title']           = wp_strip_all_tags( $new_instance['title'] );
		$instance['fooditem_id']     = wp_strip_all_tags( $new_instance['fooditem_id'] );
		$instance['display_type']    = isset( $new_instance['display_type'] )    ? wp_strip_all_tags( $new_instance['display_type'] ) : '';
