...
class RPRESS_Payments_Export extends RPRESS_Export {
	...
	public function headers() {
		...
		if ( ! rpress_is_func_disabled( 'set_time_limit' ) )
			set_time_limit( 0 );
		$month = isset( $_POST['month'] ) ? absint( $_POST['month'] ) : gmdate( 'n' );
		$year  = isset( $_POST['year']  ) ? absint( $_POST['year'] ) : gmdate( 'Y' );
		nocache_headers();
	...
	public function get_data() {
		...
		foreach ( $payments as $payment ) {
			...
			$data[] = array(
				...
				'address1' => isset( $user_info['address']['line1'] ) ? sanitize_text_field( $user_info['address']['line1'] ) : '',
				'address2' => isset( $user_info['address']['line2'] ) ? sanitize_text_field( $user_info['address']['line2'] ) : '',
				'city'     => isset( $user_info['address']['city'] ) ? sanitize_text_field( $user_info['address']['city'] ) : '',
				'state'    => isset( $user_info['address']['state'] ) ? sanitize_text_field( $user_info['address']['state'] ) : '',
				'country'  => isset( $user_info['address']['country'] ) ? sanitize_text_field( $user_info['address']['country'] ) : '',
				'zip'      => isset( $user_info['address']['zip'] ) ? sanitize_text_field( $user_info['address']['zip'] ) : '',
				'products' => $products,
				'skus'     => $skus,
