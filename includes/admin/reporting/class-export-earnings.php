...
class RPRESS_Earnings_Export extends RPRESS_Export {
	...
	public function headers() {
		...
		header( 'Content-Disposition: attachment; filename=' . apply_filters( 'rpress_earnings_export_filename', 'rpress-export-' . $this->export_type . '-' . gmdate( 'n' ) . '-' . gmdate( 'Y' ) ) . '.csv' );
		header( "Expires: 0" );
	}
	/**
	 * Set the CSV columns
	...
	public function csv_cols() {
		$cols = array(
			...
			'earnings' => esc_html__( 'Earnings', 'restropress' ) . ' (' . html_entity_decode( rpress_currency_filter( '' ) ) . ')'
		);
		return $cols;
	}
	/**
