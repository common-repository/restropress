...
class RPRESS_Batch_Customers_Export extends RPRESS_Batch_Export {
	...
	public function csv_cols() {
		$cols = array(
			...
			'purchase_count' => esc_html__( 'Number of Purchases', 'restropress' ),
			'purchase_value' => esc_html__( 'Customer Value', 'restropress' ),
			'line1'			 => esc_html__( 'Address Line1', 'restropress' ),
			'line2'			 => esc_html__( 'Address Line2', 'restropress' ),
			'country'   	 => esc_html__( 'Country/Region', 'restropress' ),
			'city'			 => esc_html__( 'City', 'restropress' ),
			'state'			 => esc_html__( 'State', 'restropress' ),
			'zip'			 => esc_html__( 'Postal Code', 'restropress' )
		);
		return $cols;
	}
	/**
		...
		if ( ! empty( $this->fooditem ) ) {
			...
			if ( $logs ) {
				foreach ( $logs as $log ) {
					...
					// Validate each field
					$line1   = isset( $customer_address['line1'] ) ? sanitize_text_field( $customer_address['line1'] ) : '';
					$line2   = isset( $customer_address['line2'] ) ? sanitize_text_field( $customer_address['line2'] ) : '';
					$country = isset( $customer_address['country'] ) ? sanitize_text_field( $customer_address['country'] ) : '';
					$city    = isset( $customer_address['city'] ) ? sanitize_text_field( $customer_address['city'] ) : '';
					$state   = isset( $customer_address['state'] ) ? sanitize_text_field( $customer_address['state'] ) : '';
					$zip     = isset( $customer_address['zip'] ) ? sanitize_text_field( $customer_address['zip'] ) : '';
