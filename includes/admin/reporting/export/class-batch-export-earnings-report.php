...
/**
 * RPRESS_Earnings_Report_Export Class
 *
 * @since 1.0
 */
class RPRESS_Batch_Earnings_Report_Export extends RPRESS_Batch_Export {
	/**
	...
	public function headers() {
		...
		header( 'Content-Disposition: attachment; filename=' . apply_filters( 'rpress_earnings_report_export_filename', 'rpress-export-' . $this->export_type . '-' . gmdate( 'm' ) . '-' . gmdate( 'Y' ) ) . '.csv' );
		header( "Expires: 0" );
	}
	/**
	...
	public function get_csv_cols() {
		...
		$pre_status_columns = array(
			esc_html__( 'Monthly Sales Activity', 'restropress' ),
			esc_html__( 'Gross Activity', 'restropress' ),
		);
		...
		$cols[] = esc_html__( 'Net Activity', 'restropress' );
		return $cols;
	}
	/**
	...
	public function get_status_cols() {
		$status_cols        = rpress_get_payment_statuses();
	...
	public function get_supported_statuses() {
		...
		return apply_filters( 'rpress_export_earnings_supported_statuses', $supported_statuses );
	}
	/**
	...
	public function print_csv_cols() {
		...
		for ( $i = 1; $i <= $number_cols; $i++ ) {
			$col_data .= esc_html__( 'Order Count', 'restropress' ) . ',';
			$col_data .= esc_html__( 'Gross Amount', 'restropress' );
			if ( $number_cols !== $i ) {
				$col_data .= ',';
			}
		}
	...
	public function print_csv_rows() {
		...
		$data = $this->get_data();
		if ( $data ) {
			$start_date = gmdate( 'Y-m-d', strtotime( $this->start ) );
			if ( $this->count() == 0 ) {
				$end_date = gmdate( 'Y-m-d', strtotime( $this->end ) );
			} else {
				$end_date = gmdate( 'Y-m-d', strtotime( 'first day of +1 month', strtotime( $start_date ) ) );
			}
			if ( $this->step == 1 ) {
				$row_data .= $start_date . ',';
			} elseif ( $this->step > 1 ) {
				$start_date = gmdate( 'Y-m-d', strtotime( 'first day of +' . ( $this->step - 1 ) . ' month', strtotime( $start_date ) ) );
				if ( gmdate( 'Y-m', strtotime( $start_date ) ) == gmdate( 'Y-m', strtotime( $this->end ) ) ) {
					$end_date = gmdate( 'Y-m-d', strtotime( $this->end ) );
					$row_data .= $end_date . ',';
				} else {
					$row_data .= $start_date . ',';
			...
			$row_data .= '"' . rpress_format_amount( $gross_amount ) . '",';
			foreach ( $data as $status => $status_data ) {
				$row_data .= isset( $data[ $status ]['count'] ) ? $data[ $status ]['count'] . ',' : 0 . ',';
				$column_amount = isset( $data[ $status ]['amount'] ) ? rpress_format_amount( $data[ $status ]['amount'] ) : 0;
			...
			$completed_statuses = apply_filters( 'rpress_export_earnings_completed_statuses', array( 'publish', 'revoked' ) );
			$net_count  = 0;
			$net_amount = 0;
			...
			$row_data .= $net_count . ',';
			$row_data .= '"' . rpress_format_amount( $net_amount ) . '"';
			$row_data .= "\r\n";
	...
	public function get_data() {
		global $wpdb;
		$data = array();
		$start_date = gmdate( 'Y-m-d 00:00:00', strtotime( $this->start ) );
		if ( $this->count() == 0 ) {
			$end_date = gmdate( 'Y-m-d 23:59:59', strtotime( $this->end ) );
		} else {
			$end_date = gmdate( 'Y-m-d 23:59:59', strtotime( 'first day of +1 month', strtotime( $start_date ) ) );
		}
		if ( $this->step > 1 ) {
			$start_date = gmdate( 'Y-m-d 00:00:00', strtotime( 'first day of +' . ( $this->step - 1 ) . ' month', strtotime( $start_date ) ) );
			if ( gmdate( 'Y-m', strtotime( $start_date ) ) == gmdate( 'Y-m', strtotime( $this->end ) ) ) {
				$end_date = gmdate( 'Y-m-d 23:59:59', strtotime( $this->end ) );
			} else {
				$end_date = gmdate( 'Y-m-d 23:59:59', strtotime( 'first day of +1 month', strtotime( $start_date ) ) );
		...
		$data = apply_filters( 'rpress_export_get_data_' . $this->export_type, $data, $start_date, $end_date );
		return $data;
	}
	/**
	...
	private function count() {
		return abs( ( gmdate( 'Y', strtotime( $this->end ) ) - gmdate( 'Y', strtotime( $this->start ) ) ) * 12 + ( gmdate( 'm', strtotime( $this->end ) ) - gmdate( 'm', strtotime( $this->start ) ) ) );
	}
	/**
