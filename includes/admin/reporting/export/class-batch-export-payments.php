...
class RPRESS_Batch_Payments_Export extends RPRESS_Batch_Export {
	...
	public function csv_cols() {
		$cols = array(
			'id'           => esc_html__( 'Payment ID',   'restropress' ), // unaltered payment ID (use for querying)
			'seq_id'       => esc_html__( 'Payment Number',   'restropress' ), // sequential payment ID
			'email'        => esc_html__( 'Email', 'restropress' ),
			'customer_id'  => esc_html__( 'Customer ID', 'restropress' ),
			'first'        => esc_html__( 'First Name', 'restropress' ),
			'last'         => esc_html__( 'Last Name', 'restropress' ),
			'address1'     => esc_html__( 'Address', 'restropress' ),
			'address2'     => esc_html__( 'Address (Line 2)', 'restropress' ),
			'city'         => esc_html__( 'City', 'restropress' ),
			'state'        => esc_html__( 'State', 'restropress' ),
			'country'      => esc_html__( 'Country', 'restropress' ),
			'zip'          => esc_html__( 'Zip / Postal Code', 'restropress' ),
	   'delivery_address'  => esc_html__('Delivery Address', 'restropress'),
			'products'     => esc_html__( 'Products (Verbose)', 'restropress' ),
			'instruction'  => esc_html__( 'Special Instruction','restropress' ),
   			'service_type' => esc_html__( 'Service Type','restropress' ),
			'service_time' => esc_html__( 'Service Time','restropress' ),
			'products_raw' => esc_html__( 'Products (Raw)', 'restropress' ),
			'skus'         => esc_html__( 'SKUs', 'restropress' ),
			'amount'       => esc_html__( 'Amount', 'restropress' ) . ' (' . html_entity_decode( rpress_currency_filter( '' ) ) . ')',
			'tips'         => esc_html__( 'Tips', 'restropress' ) . ' (' . html_entity_decode( rpress_currency_filter( '' ) ) . ')',
			'tax'          => esc_html__( 'Tax', 'restropress' ) . ' (' . html_entity_decode( rpress_currency_filter( '' ) ) . ')',
			'discount'     => esc_html__( 'Discount Code', 'restropress' ),
			'gateway'      => esc_html__( 'Payment Method', 'restropress' ),
			'trans_id'     => esc_html__( 'Transaction ID', 'restropress' ),
			'key'          => esc_html__( 'Purchase Key', 'restropress' ),
			'date'         => esc_html__( 'Date', 'restropress' ),
			'user'         => esc_html__( 'User', 'restropress' ),
			'currency'     => esc_html__( 'Currency', 'restropress' ),
			'ip'           => esc_html__( 'IP Address', 'restropress' ),
			'mode'         => esc_html__( 'Mode (Live|Test)', 'restropress' ),
			'order_status' => esc_html__( 'Status', 'restropress' ),
			'country_name' => esc_html__( 'Country Name', 'restropress' ),
		);
		if( ! rpress_use_skus() ){
			unset( $cols['skus'] );
		}
	...
	public function get_data() {
		...
		if( ! empty( $this->start ) || ! empty( $this->end ) ) {
			$args['start_date'] = gmdate( 'Y-n-d 00:00:00', strtotime( $this->start ) ) ;
			$args['end_date']   = gmdate( 'Y-n-d 23:59:59', strtotime( $this->end ) ) ;
		}
		...
		if( $payments ) {
			foreach ( $payments as $payment ) {
				...
				$tips = isset($payment_meta['fees']['tip']) ? $payment_meta['fees']['tip'] : null;
				$tips_amount = isset($tips['amount']) ? $tips['amount'] : null;
				...
				if ( $fooditems ) {
					foreach ( $fooditems as $key => $fooditem ) {
						...
						$id  = isset( $payment_meta['cart_details'] ) ? $fooditem['id'] : $fooditem;
							$title = $fooditem['name'];
							$add_items = $fooditem['item_number'];
							$item_name = array();
				...
				'delivery_address' => $user_address,
					...
					'discount'     => isset( $user_info['discount'] ) && $user_info['discount'] != 'none' ? $user_info['discount'] : esc_html__( 'none', 'restropress' ),
					...
					'date'         => $payment->date,
					'user'         => $user ? $user->display_name : esc_html__( 'guest', 'restropress' ),
	...
	public function get_percentage_complete() {
		$status = $this->status;
		$args   = array(
			'start-date' => gmdate( 'n/d/Y', strtotime( $this->start ) ),
			'end-date'   => gmdate( 'n/d/Y', strtotime( $this->end ) ),
		);
		if( 'any' == $status ) {
