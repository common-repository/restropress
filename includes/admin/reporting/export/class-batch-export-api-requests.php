...
class RPRESS_Batch_API_Requests_Export extends RPRESS_Batch_Export {
	...
	public function csv_cols() {
		$cols = array(
			'ID'      => esc_html__( 'Log ID',   'restropress' ),
			'request' => esc_html__( 'API Request', 'restropress' ),
			'ip'      => esc_html__( 'IP Address', 'restropress' ),
			'user'    => esc_html__( 'API User', 'restropress' ),
			'key'     => esc_html__( 'API Key', 'restropress' ),
			'version' => esc_html__( 'API Version', 'restropress' ),
			'speed'   => esc_html__( 'Request Speed', 'restropress' ),
	...
	public function get_data() {
		...
		if ( ! empty( $this->start ) || ! empty( $this->end ) ) {
			$args['date_query'] = array(
				array(
					'after'     => gmdate( 'Y-n-d H:i:s', strtotime( $this->start ) ),
					'before'    => gmdate( 'Y-n-d H:i:s', strtotime( $this->end ) ),
					'inclusive' => true
				)
			);
		}
