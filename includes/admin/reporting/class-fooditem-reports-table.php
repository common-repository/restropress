...
class RPRESS_Fooditem_Reports_Table extends WP_List_Table {
	...
	public function column_default( $item, $column_name ) {
		switch( $column_name ){
			...
			case 'average_earnings' :
				return rpress_currency_filter( rpress_format_amount( $item[ $column_name ] ) );
			case 'details' :
				return '<a href="' . admin_url( 'admin.php?page=rpress-reports&view=fooditems&fooditem-id=' . $item['ID'] ) . '">' . esc_html__( 'View Detailed Report', 'restropress' ) . '</a>';
			default:
	...
	public function get_columns() {
		$columns = array(
			...
			'average_sales'    => esc_html__( 'Monthly Average Sales', 'restropress' ),
			'average_earnings' => esc_html__( 'Monthly Average Earnings', 'restropress' ),
			'details'          => esc_html__( 'Detailed Report', 'restropress' ),
		);
		return $columns;
	}
	/**
