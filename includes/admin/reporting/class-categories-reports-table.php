...
/**
 * RPRESS_Categories_Reports_Table Class
 *
 * Renders the Reports table
 *
 * @since 2.4
 */
class RPRESS_Categories_Reports_Table extends WP_List_Table {
	/**
	 * Get things started
	 *
	 * @since 2.4
	...
	public function get_columns() {
		$columns = array(
			'label'          => esc_html__( 'Category', 'restropress' ),
			'total_sales'    => esc_html__( 'Total Sales', 'restropress' ),
			'total_earnings' => esc_html__( 'Total Earnings', 'restropress' ),
			'avg_sales'      => esc_html__( 'Monthly Sales Avg', 'restropress' ),
			'avg_earnings'   => esc_html__( 'Monthly Earnings Avg', 'restropress' ),
		);
		return $columns;
	}
	/**
		...
		if ( false !== $cached_reports ) {
			...
			foreach ( $categories as $category_id => $category ) {
				...
				if ( ! empty( $avg_earnings ) && $avg_sales < 1 ) {
					$avg_sales = esc_html__( 'Less than 1', 'restropress' );
				} else {
	...
	public function output_sales_graph() {
		...
		if ( empty( $total_sales ) ) {
			echo '<p><em>' . esc_html__( 'No sales for dates provided.', 'restropress' ) . '</em></p>';
		}
	...
	public function output_earnings_graph() {
		...
		if ( empty( $total_earnings ) ) {
			echo '<p><em>' . esc_html__( 'No earnings for dates provided.', 'restropress' ) . '</em></p>';
		}
		// Sort High to Low, prior to filter so people can reorder if they please
		arsort( $data );
