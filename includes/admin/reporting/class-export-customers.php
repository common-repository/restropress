...
class RPRESS_Customers_Export extends RPRESS_Export {
	...
	public function headers() {
		...
		header( 'Content-Disposition: attachment; filename=' . apply_filters( 'rpress_customers_export_filename', 'rpress-export-' . $extra . $this->export_type . '-' . gmdate( 'm-d-Y' ) ) . '.csv' );
		header( "Expires: 0" );
	}
	/**
	 * Set the CSV columns
	...
	public function csv_cols() {
		if ( ! empty( $_POST['rpress_export_fooditem'] ) ) {
			$cols = array(
				'first_name' => esc_html__( 'First Name',   'restropress' ),
				'last_name'  => esc_html__( 'Last Name',   'restropress' ),
				'email'      => esc_html__( 'Email', 'restropress' ),
				'date'       => esc_html__( 'Date Purchased', 'restropress' )
			);
		} else {
			$cols = array();
			if( 'emails' != $_POST['rpress_export_option'] ) {
				$cols['name'] = esc_html__( 'Name',   'restropress' );
			}
			$cols['email'] = esc_html__( 'Email',   'restropress' );
			if( 'full' == $_POST['rpress_export_option'] ) {
				$cols['purchases'] = esc_html__( 'Total Purchases',   'restropress' );
				$cols['amount']    = esc_html__( 'Total Purchased', 'restropress' ) . ' (' . html_entity_decode( rpress_currency_filter( '' ) ) . ')';
			}
		}
		return $cols;
	}
