...
function rpress_reports_graph() {
	...
	if ( $dates['range'] == 'today' || $dates['range'] == 'yesterday' ) {
		...
		while ( ! $day_by_day && ( strtotime( $date_start ) <= strtotime( $date_end ) ) ) {
			...
			$date_start = gmdate( 'Y-m', strtotime( '+1 month', strtotime( $date_start ) ) );
		}
		$sales_data    = array();
		$earnings_data = array();
	...
	$data = array(
		esc_html__( 'Earnings', 'restropress' ) => $earnings_data,
		esc_html__( 'Sales', 'restropress' )    => $sales_data
	);
			...
			<div class="postbox">
				...
				<div class="inside">
					...
					<p class="rpress_graph_totals">
						<strong>
							<?php
								esc_html_e( 'Total earnings for period shown: ', 'restropress' );
								echo esc_html( rpress_currency_filter( rpress_format_amount( $earnings_totals ) ) );
							?>
						</strong>
					...
					<p class="rpress_graph_totals"><strong><?php esc_html_e( 'Total sales for period shown: ', 'restropress' ); echo esc_html( rpress_format_amount( $sales_totals, false ) ); ?></strong></p>
					<?php if( ! empty( $dates['range'] ) && 'this_month' == $dates['range'] ) : ?>
						<p class="rpress_graph_totals">
							<strong>
								<?php
									esc_html_e( 'Estimated monthly earnings: ', 'restropress' );
									echo esc_html( rpress_currency_filter( rpress_format_amount( $estimated['earnings'] ) ) );
								?>
							</strong>
							<?php if ( ! $include_taxes ) : ?>
								<sup>&dagger;</sup>
						...
						<p class="rpress_graph_totals"><strong><?php esc_html_e( 'Estimated monthly sales: ', 'restropress' ); echo esc_html( rpress_format_amount( $estimated['sales'], false ) ); ?></strong></p>
					<?php endif; ?>
...
function rpress_reports_graph_of_fooditem( $fooditem_id = 0 ) {
	...
	<div class="metabox-holder" style="padding-top: 0;">
		<div class="postbox">
			<h3><span><?php printf( 
								/* translators: Fooditem Title */
								esc_html__('Earnings Over Time for %s', 'restropress' ), 
								esc_html( get_the_title( $fooditem_id ) )
							); ?></span></h3>
			<div class="inside">
				<?php
				rpress_reports_graph_controls();
				...
				$graph->display();
				?>
				<p class="rpress_graph_totals"><strong><?php esc_html_e( 'Total earnings for period shown: ', 'restropress' ); echo esc_html( rpress_currency_filter( rpress_format_amount( $earnings_totals ) ) ); ?></strong></p>
				...
				<p class="rpress_graph_totals"><strong><?php printf( 
																/* translators: Monthly fooditem earning */
																esc_html__( 'Average monthly earnings: %s', 'restropress' ), 
																esc_html( rpress_currency_filter( rpress_format_amount( rpress_get_average_monthly_fooditem_earnings( $fooditem_id ) ) ) )
															); ?>
				<p class="rpress_graph_totals"><strong><?php printf( 
																/* translators: Monthly fooditem sale */
																esc_html__( 'Average monthly sales: %s', 'restropress' ), 
																number_format( rpress_get_average_monthly_fooditem_sales( $fooditem_id ), 0 ) 
															); ?>
			</div>
		</div>
	</div>
...
function rpress_reports_graph_controls() {
	$date_options = apply_filters( 'rpress_report_date_options', array(
		...
		'last_week'    => esc_html__( 'Last Week', 'restropress' ),
		'last_30_days' => esc_html__( 'Last 30 Days', 'restropress' ),
		'this_month'   => esc_html__( 'This Month', 'restropress' ),
		'last_month'   => esc_html__( 'Last Month', 'restropress' ),
		'this_quarter' => esc_html__( 'This Quarter', 'restropress' ),
		'last_quarter' => esc_html__( 'Last Quarter', 'restropress' ),
		'this_year'    => esc_html__( 'This Year', 'restropress' ),
		'last_year'    => esc_html__( 'Last Year', 'restropress' ),
		'other'        => esc_html__( 'Custom', 'restropress' )
	) );
	$dates   = rpress_get_report_dates();
...
function rpress_get_report_dates() {
	...
	if ( 'custom' !== $dates['range'] ) {
		$dates['year']       = isset( $_GET['year'] )    ? sanitize_text_field( $_GET['year'] )     : gmdate( 'Y' );
		$dates['year_end']   = isset( $_GET['year_end'] )? sanitize_text_field( $_GET['year_end'] ) : gmdate( 'Y' );
	...
	switch ( $dates['range'] ) :
		...
		case 'this_week' :
		case 'last_week' :
			$base_time = $dates['range'] === 'this_week' ? current_time( 'mysql' ) : gmdate( 'Y-m-d h:i:s', current_time( 'timestamp' ) - WEEK_IN_SECONDS );
		...
		case 'last_30_days' :
			...
			$dates['day']      = gmdate( 'd', $date_start );
			$dates['m_start']  = gmdate( 'n', $date_start );
			$dates['year']     = gmdate( 'Y', $date_start );
...
function rpress_reports_refresh_button() {
	...
	echo '<a href="' . esc_url( $url ) . '" title="' . esc_html__( 'Clicking this will clear the reports cache', 'restropress' ) . '"  class="button rpress-refresh-reports-button">' . esc_html__( 'Refresh Reports', 'restropress' ) . '</a>';
}
