 ...
 * @since  1.0.0
 */
class RPRESS_Tools_Recount_Single_Customer_Stats extends RPRESS_Batch_Export {
	/**
	...
	public function get_data() {
		...
		$payments = $this->get_stored_data( 'rpress_recount_customer_payments_' . $customer->id, array() );
		$offset     = ( $this->step - 1 ) * $this->per_step;
		$step_items = array_slice( $payments, $offset, $this->per_step );
		if ( count( $step_items ) > 0 ) {
			$pending_total = (float) $this->get_stored_data( 'rpress_stats_customer_pending_total' . $customer->id, 0 );
			$step_total    = 0;
			$found_payment_ids = $this->get_stored_data( 'rpress_stats_found_payments_' . $customer->id, array() );
			foreach ( $step_items as $payment ) {
				$payment = get_post( $payment->ID );
				if ( is_null( $payment ) || is_wp_error( $payment ) || 'rpress_payment' !== $payment->post_type ) {
					$missing_payments   = $this->get_stored_data( 'rpress_stats_missing_payments' . $customer->id, array() );
					$missing_payments[] = $payment->ID;
					$this->store_data( 'rpress_stats_missing_payments' . $customer->id, $missing_payments );
					continue;
				}
				...
				if( true === $should_process_payment ) {
					...
					if ( apply_filters( 'rpress_customer_recount_sholud_increase_value', true, $payment ) ) {
						$payment_amount      = rpress_get_payment_amount( $payment->ID );
						$step_total         += $payment_amount;
					}
				}
			}
			$updated_total = $pending_total + $step_total;
			$this->store_data( 'rpress_stats_customer_pending_total' . $customer->id, $updated_total );
			$this->store_data( 'rpress_stats_found_payments_' . $customer->id, $found_payment_ids );
			return true;
		}
		return false;
	}
	...
	public function get_percentage_complete() {
		$payments = $this->get_stored_data( 'rpress_recount_customer_payments_' . $this->customer_id, array() );
	...
	public function process_step() {
		if ( ! $this->can_export() ) {
			wp_die( esc_html__( 'You do not have permission to modify this data.', 'restropress' ), esc_html__( 'Error', 'restropress' ), array( 'response' => 403 ) );
		}
		...
		if( $had_data ) {
			...
			$customer         = new RPRESS_Customer( $this->customer_id );
			$payment_ids      = $this->get_stored_data( 'rpress_stats_found_payments_' . $customer->id, array() );
			$this->delete_data( 'rpress_stats_found_payments_' . $customer->id );
			$removed_payments = array_unique( $this->get_stored_data( 'rpress_stats_missing_payments' . $customer->id, array() ) );
			...
			$this->delete_data( 'rpress_stats_missing_payments' . $customer->id );
			$pending_total = $this->get_stored_data( 'rpress_stats_customer_pending_total' . $customer->id, 0 );
			$this->delete_data( 'rpress_stats_customer_pending_total' . $customer->id );
			$this->delete_data( 'rpress_recount_customer_stats_' . $customer->id );
			$this->delete_data( 'rpress_recount_customer_payments_' . $this->customer_id );
	...
	public function pre_fetch() {
		if ( $this->step === 1 ) {
			...
			$customer = new RPRESS_Customer( $this->customer_id );
			$customer->update( array( 'purchase_value' => rpress_format_amount( 0 ), 'purchase_count' => 0 ) );
			...
			$this->store_data( 'rpress_recount_customer_payments_' . $customer->id, $payments );
		}
	}
	/**
