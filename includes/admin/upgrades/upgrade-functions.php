...
function rpress_trigger_upgrades() {
	if( ! current_user_can( 'manage_shop_settings' ) ) {
		wp_die( esc_html__( 'You do not have permission to do shop upgrades', 'restropress' ), esc_html__( 'Error', 'restropress' ), array( 'response' => 403 ) );
	}
...
function rpress_v134_upgrades() {
	...
	$failed = wp_insert_post(
		array(
			'post_title'     => esc_html__( 'Transaction Failed', 'restropress' ),
			'post_content'   => esc_html__( 'Your transaction failed, please try again or contact site support.', 'restropress' ),
			'post_status'    => 'publish',
			'post_author'    => 1,
			'post_type'      => 'page',
