 ...
 * file fooditem counts for a customer.
 *
 * @subpackage  Admin/Classes/RPRESS_SL_License_Log_Migration
...
/**
 * RPRESS_File_Download_Log_Migration Class
 *
 * @since 2.9.2
 */
class RPRESS_File_Download_Log_Migration extends RPRESS_Batch_Export {
	/**
	...
	public function get_data() {
		global $wpdb;
		$items = $this->get_stored_data( 'rpress_file_fooditem_log_ids' );
		if ( ! is_array( $items ) ) {
			return false;
		}
		...
		if ( $step_items ) {
			foreach ( $step_items as $log_id ) {
				...
				if ( $sanitized_log_id !== $log_id ) {
					rpress_debug_log( "Log ID mismatch, skipping log ID {$log_id}" );
					continue;
				}
				$has_customer_id = get_post_meta( $log_id, '_rpress_log_customer_id', true );
				if ( ! empty( $has_customer_id ) ) {
					continue;
				}
				$payment_id = $wpdb->get_var( "SELECT meta_value FROM {$wpdb->postmeta} WHERE post_id = {$sanitized_log_id} AND meta_key = '_rpress_log_payment_id'" );
				if ( ! empty( $payment_id ) ) {
					...
					update_post_meta( $log_id, '_rpress_log_customer_id', $customer_id );
					delete_post_meta( $log_id, '_rpress_log_user_info' );
				}
			}
			return true;
		}
	...
	public function get_percentage_complete() {
		$items = $this->get_stored_data( 'rpress_file_fooditem_log_ids', false );
		$total = count( $items );
		$percentage = 100;
		if( $total > 0 ) {
	...
	public function process_step() {
		if ( ! $this->can_export() ) {
			wp_die(
				esc_html__( 'You do not have permission to run this upgrade.', 'restropress' ),
		...
		if( $had_data ) {
			...
			$this->done    = true;
			$this->message = esc_html__( 'File fooditem logs updated successfully.', 'restropress' );
