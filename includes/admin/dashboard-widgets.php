...
function rpress_register_dashboard_widgets() {
	if ( current_user_can( apply_filters( 'rpress_dashboard_stats_cap', 'view_shop_reports' ) ) ) {
		wp_add_dashboard_widget( 'rpress_dashboard_sales', esc_html__('RestroPress Sales Summary','restropress' ), 'rpress_dashboard_sales_widget' );
	}
}
			...
			<table>
				...
				<tbody>
					<tr>
						...
						<td class="b b-earnings"><?php echo esc_html( rpress_currency_filter( rpress_format_amount( $stats->get_earnings( 0, 'this_month' ) ) )); ?></td>
					</tr>
					<tr>
						<?php $monthly_sales = $stats->get_sales( 0, 'this_month', false, array( 'publish', 'revoked' ) ); ?>
						<td class="first t monthly_sales"><?php echo esc_html( _n( 'Sale', 'Sales', $monthly_sales, 'restropress' ) ); ?></td>
			...
			<table>
				...
				<tbody>
					<tr>
						...
						<td class="b b-last-month-earnings"><?php echo esc_html( rpress_currency_filter( rpress_format_amount( $stats->get_earnings( 0, 'last_month' ) ) ) ); ?></td>
					</tr>
					<tr>
						<td class="first t sales">
							<?php $last_month_sales = $stats->get_sales( 0, 'last_month', false, array( 'publish', 'revoked' ) ); ?>
							<?php echo esc_html( _n( 'Sale', 'Sales', rpress_format_amount( $last_month_sales, false ), 'restropress' ) ); ?>
						</td>
				...
				<tbody>
					<tr>
						...
						<td class="last b b-earnings">
							...
							<?php echo esc_html(rpress_currency_filter( rpress_format_amount( $earnings_today ) )); ?>
						</td>
					</tr>
					<tr>
						<td class="t sales">
			...
			<table>
				...
				<tbody>
					<tr>
						...
						<td class="last b b-earnings"><?php echo esc_html(rpress_currency_filter( rpress_format_amount( rpress_get_total_earnings() ) )); ?></td>
					</tr>
					<tr>
						<td class="t sales"><?php esc_html_e( 'Total Sales', 'restropress' ); ?></td>
						<td class="last b b-sales"><?php echo esc_html(rpress_get_total_sales()); ?></td>
					</tr>
				</tbody>
			</table>
		</div>
				...
				<thead>
					<tr>
						<td colspan="2">
							...
							<a href="<?php echo esc_url( admin_url( 'admin.php?page=rpress-payment-history' ) ); ?>">&nbsp;&ndash;&nbsp;<?php esc_html_e( 'View All', 'restropress' ); ?></a>
						</td>
					</tr>
				</thead>
				<tbody>
					...
					foreach ( $payments as $payment ) { ?>
						<tr>
							<td class="rpress_order_label">
								<a href="<?php echo esc_url( add_query_arg( 'id', $payment->ID, admin_url( 'admin.php?page=rpress-payment-history&view=view-order-details' ) ) ); ?>">
							...
							<td class="rpress_order_price">
								<a href="<?php echo esc_url( add_query_arg( 'id', $payment->ID, admin_url( 'admin.php?page=rpress-payment-history&view=view-order-details' ) ) ); ?>">
									<span class="rpress_price_label"><?php echo esc_html(rpress_currency_filter( rpress_format_amount( $payment->total ), rpress_get_payment_currency_code( $payment->ID ) )); ?></span>
								</a>
							</td>
						</tr>
						<?php
