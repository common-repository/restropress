		...
		<div class="rpress-plugin-filter">
			...
			<?php 
				...
				$current        = isset( $_GET['status'] ) ? sanitize_text_field( $_GET['status'] ): '';
				 echo sprintf( '<a href="%s"%s>%s</a>', remove_query_arg( 'status', $base ), $current === 'all' || $current == '' ? ' class="current"' : '', esc_html__('All', 'restropress') ) .'  |  '; echo sprintf( '<a href="%s"%s>%s</a>', add_query_arg( 'status', 'active', $base ), $current === 'active' ? ' class="current"' : '', esc_html__('active', 'restropress')  );
				?> 
			</div>
		</div>
