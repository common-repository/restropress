...
function rpress_get_tools_tabs() {
	...
	$tabs['import_export'] = esc_html__( 'Import/Export', 'restropress' );
			...
			<form method="post" id="rpress-tools-recount-form" class="rpress-export-form rpress-import-export-form">
				<span>
					...
					<select name="rpress-export-class" id="recount-stats-type">
						...
						<option data-type="recount-fooditem" value="RPRESS_Tools_Recount_Download_Stats"><?php printf( esc_html__( 'Recount Earnings and Sales for a %s', 'restropress' ), rpress_get_label_singular( true ) ); ?></option>
						<option data-type="recount-all" value="RPRESS_Tools_Recount_All_Stats"><?php printf( esc_html__( 'Recount Earnings and Sales for All %s', 'restropress' ), rpress_get_label_plural( true ) ); ?></option>
					...
					<span class="rpress-recount-stats-descriptions">
						...
						<span id="recount-fooditem"><?php printf( esc_html__( 'Recalculates the earnings and sales stats for a specific %s.', 'restropress' ), rpress_get_label_singular( true ) ); ?></span>
						<span id="recount-all"><?php printf( esc_html__( 'Recalculates the earnings and sales stats for all %s.', 'restropress' ), rpress_get_label_plural( true ) ); ?></span>
						...
						<?php foreach( $has_beta as $slug => $product ) : ?>
							<tr>
								...
								<td>
									...
									<label for="enabled_betas[<?php echo esc_attr( $slug ); ?>]"><?php printf( esc_html__( 'Get updates for pre-release versions of %s', 'restropress' ), $product ); ?></label>
								</td>
							</tr>
						<?php endforeach; ?>
					</tbody>
			...
			<form id="rpress-import-payments" class="rpress-import-form rpress-import-export-form" action="<?php echo esc_url( add_query_arg( 'rpress_action', 'upload_import_file', admin_url() ) ); ?>" method="post" enctype="multipart/form-data">
				...
				<div class="rpress-import-options" id="rpress-import-payments-options" style="display:none;">
					<p>
						<?php
						printf(
							esc_html(esc_html__( 'Each column loaded from the CSV needs to be mapped to a order field. Select the column that should be mapped to each field below. Any columns not needed can be ignored.', 'restropress' )
							)
						);
						?>
					</p>
			...
			<form id="rpress-import-customers" class="rpress-import-form rpress-import-export-form" action="<?php echo esc_url( add_query_arg( 'rpress_action', 'upload_import_file', admin_url() ) ); ?>" method="post" enctype="multipart/form-data">
				...
				<div class="rpress-import-options" id="rpress-import-customers-options" style="display:none;">
					<p>
						<?php
						printf(
							esc_html(esc_html__( 'Each column loaded from the CSV needs to be mapped to a customer field. Select the column that should be mapped to each field below. Any columns not needed can be ignored.', 'restropress' )
							)
						);
						?>
					</p>
			...
			<form id="rpress-import-fooditems" class="rpress-import-form rpress-import-export-form" action="<?php echo esc_url( add_query_arg( 'rpress_action', 'upload_import_file', admin_url() ) ); ?>" method="post" enctype="multipart/form-data">
				...
				<div class="rpress-import-options" id="rpress-import-fooditems-options" style="display:none;">
					<p>
						<?php
						printf(
							esc_html(esc_html__( 'Each column loaded from the CSV needs to be mapped to a Food Item field. Select the column that should be mapped to each field below. Any columns not needed can be ignored.', 'restropress' )
							)
						);
						?>
					</p>
