...
<div class="wrap rpress-wrap">
	<h2>
		...
		if ( rpress_get_option( 'enable_sequential' ) ) {
			printf(
				/* translators: %s: Placeholder for sequence number */
				esc_html__( 'Order #%s', 'restropress' ), 
				esc_html( $number )
			); 
		} else {
			printf( 
				/* translators: %s: Placeholder for order id */
				esc_html__( 'Order #%s', 'restropress' ), 
				esc_html( $order_id )
			);
		}
			...
			<div class="rpress-admin-box">
				...
				<div class="rpress-admin-box-inside">
					<p>
						...
						<?php
						$status_help  = '<ul>';
						$status_help .= '<li>' . esc_html__( '<strong>Pending</strong>: payment is still processing or was abandoned by customer. Successful payments will be marked as Complete automatically once processing is finalized.', 'restropress' ) . '</li>';
						$status_help .= '<li>' . esc_html__( '<strong>Complete</strong>: all processing is completed for this purchase.', 'restropress' ) . '</li>';
						$status_help .= '<li>' . esc_html__( '<strong>Revoked</strong>: access to purchased items is disabled, perhaps due to policy violation or fraud.', 'restropress' ) . '</li>';
						$status_help .= '<li>' . esc_html__( '<strong>Refunded</strong>: the purchase amount is returned to the customer and access to items is disabled.', 'restropress' ) . '</li>';
						$status_help .= '<li>' . esc_html__( '<strong>Abandoned</strong>: the purchase attempt was not completed by the customer.', 'restropress' ) . '</li>';
						$status_help .= '<li>' . esc_html__( '<strong>Failed</strong>: customer clicked Cancel before completing the purchase.', 'restropress' ) . '</li>';
						$status_help .= '</ul>';
						?>
					...
					<div class="rpress-admin-box-inside">
						<p>
							...
							<input type="text" name="rpress-payment-date" value="<?php echo esc_attr( gmdate( 'm/d/Y', $payment_date ) ); ?>" class="medium-text rpress_datepicker"/>
						</p>
					</div>
					<div class="rpress-admin-box-inside">
						<p>
							...
							<input type="text" maxlength="2" name="rpress-payment-time-min" value="<?php echo esc_attr( gmdate( 'i', $payment_date ) ); ?>" class="small-text rpress-payment-time-min"/>
						</p>
					</div>
                    <div class="rpress-admin-box-inside">
						...
						<p class="rpress-order-fees strong">
							...
							<ul class="rpress-payment-fees">
								<?php foreach( $fees as $fee ) : ?>
									<li data-fee-id="<?php echo esc_attr( $fee['id'] ); ?>"><span class="fee-label"><?php echo esc_html( $fee['label'] ) . ':</span> ' . '<span class="fee-amount" data-fee="' . esc_attr( $fee['amount'] ) . '">' . esc_html( rpress_currency_filter( $fee['amount'], $currency_code ) ); ?></span></li>
								<?php endforeach; ?>
							</ul>
						</p>
					</div>
                    ...
                    <?php if ( !empty( $discount ) ) : ?>
						<div class="rpress-admin-box-inside">
							<p class="rpress-order-discount">
								<span class="label"><?php esc_html_e( 'Coupon', 'restropress' ); esc_html_e(' ('. $discount_code. ')', 'restropress' ); ?>:</span>&nbsp;
	...
	<div id="rpress-payment-notes" class="postbox">
		...
		<div class="inside">
			<div id="rpress-payment-notes-inner">
				...
				echo '<p class="rpress-no-payment-notes"' . esc_attr( $no_notes_display ). '>'. esc_html__( 'No payment notes', 'restropress' ) . '</p>'; ?>
			</div>
			...
			<div class="inside rpress-clearfix">
						...
						<div class="column">
							<div class="rpress-delivery-address">
								<h3><?php echo sprintf( 
									...
									esc_html__( '%s address:' ), 
									rpress_service_label( $service_type ) 
									);?></h3>
				...
				<?php if( !empty( $order_note ) ) : ?>
					<div class="column-container customer-instructions">
						<h3><?php echo sprintf( esc_html__( '%s instructions:' ), rpress_service_label( $service_type ) );?></h3>
    		...
    		<div id="rpress-purchased-items" class="postbox rpress-edit-purchase-element <?php echo esc_attr( $column_count ); ?>">
    			<div class="rpress-purchased-items-header row header">
    				<ul class="rpress-purchased-items-list-header">
                    	...
                    	<li class="item_gross_price"><?php printf( _x( '%s Gross Total', 'payment details purchased item gross total - full screen', 'restropress' ), rp_get_label_singular() ); ?>
                    	</li>
                    </ul>
                </div>
                <?php
                      		...
                      		<ul class="rpress-purchased-items-list-wrapper <?php echo esc_attr( $key ); ?>">
                      			<li class="fooditem">
                      				...
                      				<span class="rpress-purchased-fooditem-title">
                      					...
                      					<?php else: ?>
                  							<span class="deleted">
                  								...
                  								<?php else: ?>
                  									<em><?php printf( esc_html__( '%s deleted', 'restropress' ), rpress_get_label_singular() ); ?></em>
                  								<?php endif; ?>
                  							</span>
                  						<?php endif; ?>
                  					</span>
                  					...
                  					<?php if ( ! empty( $cart_items[ $key ]['fees'] ) ) :
                  						...
                  						<input type="hidden" name="rpress-payment-details-fooditems[<?php echo $key; ?>][fees]" class="rpress-payment-details-fooditem-fees" value="<?php echo esc_attr( wp_json_encode( $fees ) ); ?>"/>
                  					<?php endif; ?>
                  				</li>
                  				<li class="item_price">
                  				...
                  				<li class="item_discount">
                  					...
                  					<?php echo rpress_currency_symbol( $currency_code ); ?>
                  					<input type="text" class="small-text rpress-price-field rpress-payment-details-fooditem-item-discount rpress-payment-item-input rpress-order-input" name="rpress-payment-details-fooditems[<?php echo $key; ?>][item_discount]" value="<?php echo rpress_format_amount( $item_discount ); ?>" />
                  				</li>
                  				<?php endif; ?>
	...
	<div class="food-item-list">
		...
		<?php else : $key = 0; ?>
			<div class="row">
				<p><?php printf( esc_html__( 'No %s included with this purchase', 'restropress' ), rp_get_label_plural() ); ?></p>
			</div>
        <?php endif; ?>
        <div class="postbox rpress-edit-purchase-element rp-add-update-elements <?php echo esc_attr( $column_count ); ?>">
        	<div class="rpress-add-fooditem-to-purchase-header row header">
        		<ul class="rpress-purchased-items-list-wrapper">
        			<li class="fooditem"><?php printf( esc_html__( 'Add New %s', 'restropress' ), rpress_get_label_singular() ); ?></li>
        	...
        	<div class="rpress-add-fooditem-to-purchase aa inside">
        		<ul>
					...
					<li class="rpress-add-fooditem-to-purchase-actions actions">
						...
						<a href="" id="rpress-order-add-fooditem" class="button button-secondary"><?php printf( esc_html__( 'Add New %s', 'restropress' ), rpress_get_label_singular() ); ?></a>
					</li>
				</ul>
