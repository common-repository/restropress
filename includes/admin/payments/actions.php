...
function rpress_update_payment_details( $data ) {
	...
	if ( isset( $data['rpress-new-customer'] ) && $data['rpress-new-customer'] == '1' ) {
		...
		if ( empty( $email ) || empty( $names ) ) {
			wp_die( esc_html__( 'New Customers require a name and email address', 'restropress' ) );
		}
		...
		if ( empty( $customer->id ) ) {
			...
			if ( ! $customer->create( $customer_data ) ) {
				...
				rpress_set_error( 'rpress-payment-new-customer-fail', esc_html__( 'Error creating new customer', 'restropress' ) );
			}
		}
 ...
 * processed. This processing cannot happen within the `RPRESS_Payment_History_Table`
