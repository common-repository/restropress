...
class RPRESS_Discount_Codes_Table extends WP_List_Table {
	...
	public function get_views() {
		...
		$views = array(
			...
			'inactive' => sprintf( '<a href="%s"%s>%s</a>', add_query_arg( 'status', 'inactive', $base ), $current === 'inactive' ? ' class="current"' : '', esc_html__( 'Inactive', 'restropress' ) . $inactive_count ),
		);
		return $views;
	}
	/**
	...
	public function get_columns() {
		$columns = array(
			...
			'code'       => esc_html__( 'Code', 'restropress' ),
			'amount'     => esc_html__( 'Amount', 'restropress' ),
			'uses'       => esc_html__( 'Uses', 'restropress' ),
			'start_date' => esc_html__( 'Start Date', 'restropress' ),
			'expiration' => esc_html__( 'Expiration', 'restropress' ),
			'status'     => esc_html__( 'Status', 'restropress' ),
		);
		return $columns;
	}
	/**
	...
	function column_name( $item ) {
		...
		$row_actions  = array();
		$row_actions['edit'] = '<a href="' . esc_url( add_query_arg( array( 'rpress-action' => 'edit_discount', 'discount' => $discount->ID ) ) ) . '">' . esc_html__( 'Edit', 'restropress' ) . '</a>';
		if( strtolower( $item['status'] ) == 'active' ) {
			$row_actions['deactivate'] = '<a href="' . esc_url( wp_nonce_url( add_query_arg( array( 'rpress-action' => 'deactivate_discount', 'discount' => $discount->ID ) ), 'rpress_discount_nonce' ) ) . '">' . esc_html__( 'Deactivate', 'restropress' ) . '</a>';
		} elseif( strtolower( $item['status'] ) == 'inactive' ) {
			$row_actions['activate'] = '<a href="' . esc_url( wp_nonce_url( add_query_arg( array( 'rpress-action' => 'activate_discount', 'discount' => $discount->ID ) ), 'rpress_discount_nonce' ) ) . '">' . esc_html__( 'Activate', 'restropress' ) . '</a>';
		}
		$row_actions['delete'] = '<a href="' . esc_url( wp_nonce_url( add_query_arg( array( 'rpress-action' => 'delete_discount', 'discount' => $discount->ID ) ), 'rpress_discount_nonce' ) ) . '">' . esc_html__( 'Delete', 'restropress' ) . '</a>';
	...
	public function get_bulk_actions() {
		$actions = array(
			'activate'   => esc_html__( 'Activate', 'restropress' ),
			'deactivate' => esc_html__( 'Deactivate', 'restropress' ),
		...
		if ( $discounts ) {
			foreach ( $discounts as $discount ) {
				...
				if ( ! empty( $start_date ) ) {
					...
					$discount_start_date = esc_html__( 'No start date', 'restropress' );
				}
				if ( rpress_get_discount_expiration( $discount->ID ) ) {
					...
					$expiration = esc_html__( 'No expiration', 'restropress' );
				}
				$discount_codes_data[] = array(
					'ID'         => $discount->ID,
