...
function rpress_edit_discount( $data ) {
	...
	if( ! current_user_can( 'manage_shop_discounts' ) ) {
		wp_die( esc_html__( 'You do not have permission to edit discount codes', 'restropress' ), esc_html__( 'Error', 'restropress' ), array( 'response' => 403 ) );
	}
	// Setup the discount code details
	$discount = array();
	foreach ( $data as $key => $value ) {
		if ( $key === 'products' || $key === 'excluded-products' ) {
			...
			if ( is_string( $value ) || is_int( $value ) ) {
				$discount[ $key ] = wp_strip_all_tags( addslashes( $value ) );
			} elseif ( is_array( $value ) ) {
...
function rpress_delete_discount( $data ) {
	if ( ! isset( $data['_wpnonce'] ) || ! wp_verify_nonce( $data['_wpnonce'], 'rpress_discount_nonce' ) ) {
		wp_die( esc_html__( 'Trying to cheat or something?', 'restropress' ), esc_html__( 'Error', 'restropress' ), array( 'response' => 403 ) );
	}
	if( ! current_user_can( 'manage_shop_discounts' ) ) {
		wp_die( esc_html__( 'You do not have permission to delete discount codes', 'restropress' ), esc_html__( 'Error', 'restropress' ), array( 'response' => 403 ) );
	}
...
function rpress_deactivate_discount( $data ) {
	...
	if( ! current_user_can( 'manage_shop_discounts' ) ) {
		wp_die( esc_html__( 'You do not have permission to create discount codes', 'restropress' ), array( 'response' => 403 ) );
	}
	$id = absint( $data['discount'] );
