	...
	public function show_notices() {
		...
		if ( ( rpress_get_option( 'purchase_page', '' ) == '' || 'trash' == get_post_status( rpress_get_option( 'purchase_page', '' ) ) ) && current_user_can( 'edit_pages' ) && ! get_user_meta( get_current_user_id(), '_rpress_set_checkout_dismissed' ) ) {
			...
			<div class="error">
				<p><?php printf(
				 ...
				 /* translators: %s is the link to the settings page */
                 wp_kses( 
                    __( 'No checkout page has been configured. Visit <a href="%s">Settings</a> to set one.', 'restropress' ), 
                    array( 
                        'a' => array( 'href' => array() )  // Allow only <a> tags with href attribute
                    ) 
                ), 
		...
		if ( class_exists( 'RPRESS_Recount_Earnings' ) && current_user_can( 'manage_shop_settings' ) ) {
			...
			<div class="error">
				<p><?php 
				printf(
				/* translators: %1$s and %2$s are the links for the recount tool and deactivation page */
                wp_kses( 
                    __( 
                        'RestroPress 2.5 contains a <a href="%1$s">built-in recount tool</a>. Please <a href="%2$s">deactivate the RestroPress - Recount Earnings plugin</a>', 
                        'restropress' 
                    ), 
                    array( 
                        'a' => array( 
                            'href' => array()  // Allow only <a> tags with href attribute
                        ) 
                    ) 
                ), 
		        esc_url(admin_url('admin.php?page=rpress-tools&tab=general')),
