...
function rpress_get_users_ordered_products( $user = 0, $status = 'complete' ) {
	...
	$args = apply_filters(
		'rpress_get_users_ordered_products_args',
		array(
			'include'        => $product_ids,
			'post_type'      => 'fooditem',
			'posts_per_page' => -1,
...
function rpress_new_user_notification( $user_id = 0, $user_data = array() ) {
	...
	$admin_subject = apply_filters(
		'rpress_user_registration_admin_email_subject',
		sprintf(
		/* translators: %s: Site name */
			__( '[%s] New User Registration', 'restropress' ),
			$from_name
		),
		$user_data
	);

	$admin_heading = apply_filters(
		'rpress_user_registration_admin_email_heading',
		/* translators: New user registration */
		__( 'New user registration', 'restropress' ),
		$user_data
	);

	$admin_message = sprintf(
		/* translators: 1: Username, 2: User email */
		__( 'Username: %1$s', 'restropress' ),
		$user_data['user_login']
	) . "\r\n\r\n";

	$admin_message .= sprintf(
		/* translators: 1: Username, 2: User email */
		__( 'E-mail: %s', 'restropress' ),
	...
	$user_subject = apply_filters(
		'rpress_user_registration_email_subject',
		sprintf(
		/* translators: %s: Site name */
			__( '[%s] Your username and password', 'restropress' ),
			$from_name
		),
		$user_data
	);
	...
	$user_message = apply_filters(
		'rpress_user_registration_email_username',
		sprintf(
		/* translators: 1: Username, 2: User email */
			__( 'Username: %s', 'restropress' ),
			$user_data['user_login']
		) . "\r\n",
		$user_data
	);
	...
	$user_message .= apply_filters(
		'rpress_user_registration_email_password',
		sprintf(
		/* translators: %s: Generated password */
			__( 'Password: %s', 'restropress' ),
			'[' . $password_message . ']'
		) . "\r\n"
	);
	...
	if ( $emails->html ) {
		...
		$user_message .= sprintf(
			...
			__( 'To log in, visit: %s', 'restropress' ),
			$login_url
		) . "\r\n";
	}
...
function rpress_validate_user_verification_token( $url = '' ) {
	...
	if ( isset( $parts['query'] ) ) {
		...
		if ( isset( $query_args['ttl'] ) && current_time( 'timestamp' ) > $query_args['ttl'] ) {
			...
			$link_text = sprintf(
				/* translators: %s: Placeholder for verification url */
			...
			wp_die( esc_html( apply_filters( 'rpress_verification_link_expired_text', $link_text ) ), esc_html__( 'Error', 'restropress' ), array( 'response' => 403 ) );
		}
...
function rpress_process_user_verification_request() {
	...
	if ( ! is_user_logged_in() ) {
		wp_die( esc_html__( 'You must be logged in to verify your account.', 'restropress' ), esc_html__( 'Notice', 'restropress' ), array( 'response' => 403 ) );
	}
	if ( ! rpress_user_pending_verification( get_current_user_id() ) ) {
		wp_die( esc_html__( 'Your account has already been verified.', 'restropress' ), esc_html__( 'Notice', 'restropress' ), array( 'response' => 403 ) );
	}
...
function rpress_process_user_account_verification() {
	...
	if ( ! rpress_validate_user_verification_token( $url ) ) {
		...
		wp_die( esc_html__( 'Invalid verification token provided.', 'restropress' ), esc_html__( 'Error', 'restropress' ), array( 'response' => 403 ) );
	}
...
if ( ! function_exists( 'rpress_get_user_all_api_keys' ) ) {
	function rpress_get_user_all_api_keys( WP_User $profile ) {
		...
		?>
		<h1>
			<?php esc_html_e( 'Restropress API Keys Table', 'restropress' ); ?>
		</h1>
		<table class="wp-list-table widefat fixed striped table-view-list">
			<thead>
				<tr>
					<th> <?php esc_html_e( 'Private Key', 'restropress' ); ?> </th>
					<th> <?php esc_html_e( 'Public Key', 'restropress' ); ?> </th>
					<th> <?php esc_html_e( 'Token', 'restropress' ); ?> </th>
			...
			<tbody>
				<tr>
					<td>
						<p><?php echo esc_html( get_user_meta( $profile->ID, '_rp_api_user_private_key', true ) ); ?></p>
					</td>
					<td>
						<p><?php echo esc_html( get_user_meta( $profile->ID, '_rp_api_user_public_key', true ) ); ?></p>
					</td>
					<td>
						<p><?php echo esc_html( get_user_meta( $profile->ID, '_rp_api_user_token_key', true ) ); ?></p>
					</td>
					<td>
						<label for="rp-api-revoke-keys"><?php esc_html_e( 'Revoke', 'restropress' ); ?></label>
						<input type="checkbox"  name="rp-api-revoke-keys" id="rp-api-revoke-keys" value="revoke"/>
						<label for="rp-api-refresh-token-keys"><?php esc_html_e( 'Refress Token', 'restropress' ); ?></label>
