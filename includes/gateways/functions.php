...
function rpress_get_chosen_gateway() {
	...
	if ($chosen === false) {
        $chosen = isset($_COOKIE['payment_mode']) ? $_COOKIE['payment_mode'] : false;
    }
    if ( false !== $chosen ) {
...
function rpress_get_total_earnings_by_gateway_with_date_range( $gateway_id = 'paypal', $start_date = null, $end_date = null ) {
    ...
    if ( $start_date === $end_date ) {
        // If start and end dates are the same, adjust end date to end of day
...
function rpress_cash_on_delivery_payment( $purchase_data ) {
	...
	if ( $payment ) {
		...
		rpress_record_gateway_error( __( 'Payment Error', 'restropress' ), sprintf( __( 'Payment creation failed while processing with Cash On delivery order. Payment data: %s', 'restropress' ), wp_json_encode( $payment_data ) ), $payment );
