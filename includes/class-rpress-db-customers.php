...
/**
 * RPRESS_DB_Customers Class
 *
 * @since  1.0.0
 */
class RPRESS_DB_Customers extends RPRESS_DB  {
	/**
	 * The metadata type.
	 *
	 * @since 1.0.0
	...
	*/
	public function __construct() {
		global $wpdb;
		$this->table_name  = $wpdb->prefix . 'rpress_customers';
		$this->primary_key = 'id';
		$this->version     = '1.0';
	...
	*/
	 ...
	 * the payment meta and logs. Use rpress_customer_delete() instead
	 *
	 * @since   2.3.1
	*/
	...
	*/
				...
				if( $success ) {
					...
					if( ! empty( $payments_array ) ) {
						foreach ( $payments_array as $payment_id ) {
							rpress_update_payment_meta( $payment_id, 'email', $user->user_email );
						}
					}
					do_action( 'rpress_update_customer_email_on_user_update', $user, $customer );
				}
			}
		}
	}
	...
	public function get_customer_by( $field = 'id', $value = 0 ) {
		...
		$query = new RPRESS_Customer_Query( '', $this );
	...
	*/
	...
	/**
	 * Prepare query arguments for `RPRESS_Customer_Query`.
	 *
	 ...
	 * @since 1.0.0
	 *
	 * @param array $args Arguments for `RPRESS_Customer_Query`.
	...
	protected function prepare_customer_query_args( $args ) {
		...
		if ( ! empty( $args['date'] ) ) {
			...
			if ( is_array( $args['date'] ) ) {
				$date_query[] = array(
					'after'     => gmdate( 'Y-m-d 00:00:00', strtotime( $args['date']['start'] ) ),
					'inclusive' => true,
				);
				$date_query[] = array(
					'before'    => gmdate( 'Y-m-d 23:59:59', strtotime( $args['date']['end'] ) ),
					'inclusive' => true,
				);
			} else {
				$date_query[] = array(
